...
{  
  ...
  // Id identificador, endereço do servidos e porta 
   success = sendData("AT+CIPSTART=2,\"TCP\",\"" + servidor + "\",7000\r\n",1000,DEBUG);//5000

  okay = resposta(success);

  if(!okay){
  Serial.println("Passou por dentro deste IF!!");
  tried = tryagain("AT+CIPSTART=\"TCP\",\"" + servidor + "\",7000\r\n");
  if(!tried){
    return;
    }
  }
