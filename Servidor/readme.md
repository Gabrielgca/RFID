 ...
 * Próximo do problema citado acima, tem-se o fato das sessões do SQLAlchemy não serem encerradas logo após sua comunicação com o banco de dados, assim, sobrecarregando o servidor quando o mesmo tentava se conectar com o banco de dados que já havia excedido seu máximo de sessões. Como solução, foi implementado uma rotina no código em que, uma sessão sempre será fechada depois que a comunicação com o banco não for mais necessária.
