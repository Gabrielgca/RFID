...
static const u4_t DEVADDR = 0x2602137F;
...
static bool newCard = false;						//variável de controle usada para checar se há um cartão novo no leitor
...
void do_send (osjob_t* j)
{
	...
	{
		//prepara um pacote de uplink para a próxima janela de transmissão disponível
...
void loop ()
{
	//rotina do módulo LoRa que envia e recebe dados
	os_runloop_once ();
  
  //verifica se existe uma tag presente
	if (!RFID.PICC_IsNewCardPresent ())
	{
		//se não, newCard recebe o valor falso e o loop volta para o começo
		newCard = false;
		return;
	}
	...
	//caso contrário, newCard recebe o valor verdadeiro e o loop prossegue
	newCard = true;
  ...
  Serial.print ("\n");
	...
	//envia o ID para a TTN e aciona o led verde para confirmação
  do_send (&sendjob);
	tagIdentified ();
	
	//aguarda antes de voltar à rotina
  delay(1000);
}
